...
import { parseInterval, RationalInterval, Rational } from '../index.js';
...
const rel4a = parseInterval('2.0[+0.1:-0.2]');
const rel4b = parseInterval('2.0[-0.2:+0.1]');
...
const add1 = parseInterval('1.23[+0.5:-0.3]')
  .add(parseInterval('2.45[+0.2:-0.1]'));
console.log(`   1.23[+0.5:-0.3] + 2.45[+0.2:-0.1] = ${add1.toString()}`);
console.log(`   Decimal: ${add1.low.toDecimal()} : ${add1.high.toDecimal()}`);

const mult1 = parseInterval('2[+0.1:-0.1]')
  .multiply(parseInterval('3[+0.2:-0.2]'));
...
const complex1 = parseInterval('1.23[+0.1:-0.1]')
  .add(parseInterval('2.45[+0.05:-0.05]'))
  .multiply(new Rational('0.5'));
...
const complex2 = parseInterval('1.23[56:67]')
  .divide(parseInterval('2[+0.1:-0.1]'));
